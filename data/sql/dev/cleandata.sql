...
DELETE FROM role_rights WHERE roleid IN (SELECT id FROM roles WHERE name IN ('FacilityHead', 'ReportViewer')) ;
DELETE FROM roles WHERE name IN ('FacilityHead', 'ReportViewer');
